(function() {
  ...
  window.quickSave = function() {
      ...
      localStorage['pageant_save_q'] = saveString;
      window.alert("Saved.");
  };

  window.saveSlot = function(slot) {
      var saveString = JSON.stringify(window.dendryUI.dendryEngine.getExportableState());
      localStorage['pageant_save_' + slot] = saveString;
      var date = new Date(Date.now());
      localStorage['pageant_save_timestamp_' + slot] = date;
      window.populateSaveSlots(slot + 1);
  };

  window.quickLoad = function() {
      if (localStorage['pageant_save_q']) {
          var saveString = localStorage['pageant_save_q'];
  ...
  window.loadSlot = function(slot) {
      if (localStorage['pageant_save_' + slot]) {
          var saveString = localStorage['pageant_save_' + slot];
          window.dendryUI.dendryEngine.setState(JSON.parse(saveString));
  ...
  window.deleteSlot = function(slot) {
      if (localStorage['pageant_save_' + slot]) {
          localStorage['pageant_save_' + slot] = '';
          localStorage['pageant_save_timestamp_' + slot] = '';
          window.populateSaveSlots(slot + 1);
      } else {
          window.alert("No save available.");
      }
  ...
  window.populateSaveSlots = function(max_slots) {
      ...
      for (var i = 0; i < max_slots; i++) {
          ...
          if (localStorage['pageant_save_' + i]) {
              var timestamp = localStorage['pageant_save_timestamp_' + i];
